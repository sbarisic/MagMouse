...
- `CAM-review-attachments.zip`: consolidated, hashed CAM review attachments.
...
- `Panel-Gerbers-QUOTE.zip`: copper, mask, silkscreen, routed edges and drills.
- `Stencil-QUOTE.zip`: top paste with U32 and hand-fitted parts excluded.
- `one-set-purchasing.csv`: exact MPN groups, references, quantities,
  minimum purchases, spares and public supplier observations.
- `all-vias.csv`: via-only fill/cap inventory. Never fill component holes,
...
- `button-calibration.md`: fixed magnet endpoint calibration; runnable firmware is separate.
- `harness-pin-map.csv`: all 46 wires, component-side and mirrored solder-side views.
...
- `evidence/acceptance.json`: fresh native and strict electrical checks.
- `copper-review/`: exported copper and reference-plane views.
